...
interface NamespaceBuilder {
    ...
    fun Property(name: String) = Declaration(ExportDeclaration.Property(childName(name), Signature.None))

    fun Function(name: String) = Declaration(ExportDeclaration.Function(childName(name), Signature.None, null, null, emptyList(), emptyMap()))

    fun Typealias(name: String) = Declaration(ExportDeclaration.Typealias(childName(name), Signature.None))
}
...
class ClassBuilder(val fqName: ResolvedName) : NamespaceBuilder {
    ...
    fun build() = DeclarationTree.Class(ExportDeclaration.Class(fqName, Signature.None), declarations)

    fun Constructor(name: String = "<init>") = Declaration(ExportDeclaration.Constructor(childName(name), Signature.None))
}
